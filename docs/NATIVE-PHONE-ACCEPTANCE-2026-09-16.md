...
| JSON 實機匯入 | 使用者手動選檔後，由 APP 確認視窗完成同一份備份匯入；2／1／4 不變，資料及 tombstones 正規化 SHA-256 與匯入前一致；同步及提醒依設計停用 |
| 回復匯入前資料 | APP 回復視窗可讀上一份 2／1／4；確認後資料與 tombstones 摘要仍一致。本次前後內容相同，尚不能代表不同版本資料的復原驗收 |
| 非空資料覆裝 | 同簽章、同版本 APK 以 install -r 重裝並重開，資料與 tombstones 摘要一致；不等於不同版本 schema 升級或 TWA 原地移轉 |
...
待辦：不同內容的回復、真實網站 JSON → 原生 → JSON → 網站移轉、離線重試、跨帳號拒絕、不同版本更新保留、最新版 Android 16 回歸與正式套件簽署。正式 `tw.searchbefore.app` Firebase／Play 簽章設定仍等待使用者同意；預覽設定的同意不等同正式設定授權。後台私人資料唯讀核對另等待確認，不改用其他通道繞過後台的安全攔截。

手機保留一筆 `TEST_ONLY_NATIVE_20260916` 設備紀錄，動作 `Sync_test_only_NOT_actual_farm_work`，以及 `Download/噴前查原生備份_2026-09-16.json`。測試通知已關閉，匯入後同步暫停，未刪除原紀錄。備份與私人畫面不加入 Git。測試期間鍵盤由注音切換為英文以輸入測試標記，已恢復注音並收起鍵盤。

程式提交 `b93131c` 的 GitHub Node CI 通過（24s），Android build／test／lint 通過（4m52s；run 35088994815）。本次續驗只更新驗收文件，沒有修改 APK 程式。
