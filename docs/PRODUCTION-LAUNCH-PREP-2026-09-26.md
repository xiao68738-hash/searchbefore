...
續作：手機連線恢復，已完成 [v9 Play 原地更新、完整資料比對與同步／重啟](NATIVE-PLAY-V9-2026-09-26.md)。以下舊的 v8／離線敘述為早先檢查紀錄，不再代表手機現況。提交 `9e77ffff79b6b63134a7e1736fe230004f055138` 的 Review 36217998545 與 Native 36217998560 均 SUCCESS。

## 現在在哪裡

- 每日 Console／Threads 監管已依使用者要求刪除，不再定時執行。既有監控紀錄保留。
- 程式來源：`agent/native-preview-20260916`，PR #148 仍 OPEN／draft。9/26 確認 `4258534` 的 Review 36216172567／Native 36216172566 均 SUCCESS；本輪新增的v10與備份比對工具需另看新提交CI，不能沿用。
- Play 9/26實際查證最新版本頁：Alpha v4、內部 v9／1.1.4-internal；v9 9/22 23:08 發布。本輪未發布／送審／修改名單；正式軌道仍未操作。
...
- 本輪已完成[v10候選](NATIVE-V10-CANDIDATE-2026-09-26.md)及[Console申報核對](PLAY-DISCLOSURE-AUDIT-2026-09-26.md)。v10僅本機，尚未安裝或上傳。

## 發行順序：先完成前項，才進下一項

| 順序 | 工作 | 可完成條件 | 目前狀態 |
|---|---|---|---|
| 1 | Play 原地升級 | 保留首次安裝時間、帳號及資料；匯出完整 JSON 前後逐欄比對，再驗證手動同步／重啟 | v8→v9、v9→v10 均已獨立完成；登入及同步同意保留，兩版各兩次完整 data 比對一致 |
| 2 | 審查登入資訊 | 隔離環境驗證專用審查帳號；Console 聲明部分功能需登入並提供英文步驟 | 使用者本輪確認尚未準備；憑證直接填 Console，不傳聊天或 Git |
| 3 | 正式識別與申報 | OAuth 顯示品牌、存取權、資料安全、隱私／刪除網址與最終依賴一致 | 已發現登入聲明錯誤與公開政策落後；SDK及資料類型已列核對表，仍需完成最終審查 |
| 4 | 最終正式候選 | 確認可用版號，移除內部／預覽標示；重建、簽章、Manifest、測試、Play 驗收 | v10／1.1.4 已於9/26 13:11僅內部發布，Play實機升級／同步重啟／完整資料驗收通過；界限見 NATIVE-PLAY-V10-2026-09-26.md |
| 5 | 正式商店素材 | 最終候選的手機與平板實際畫面；版本與功能吻合、无私人資料 | 已有五張v10實機素材候選，逐張檢視無私人資料；尚需最終選圖與平板素材，未上傳商店 |
| 6 | 品質與最終放行 | 查閱可用的預先發布報告／vitals；結合版本驗收與已知問題作判斷 | 最後讀取仍無報告／資料；不能當成零當機或自動通過 |
| 7 | 合併與正式發布 | 最終差異審查、CI、發布內容確認及明確正式發布授權 | PR148仍草稿；本輪僅內部測試發布，不合併或發布正式版 |
...
下列46檔／71tests為前一輪；續作新增後為47根JS檔、18 suites／74 release tests全過，詳見v10候選紀錄。
