...
- 表單 OCR 已停止瀏覽器本機模型，改為 Cloud Run／Google Cloud Vision 後端。現在以 `development` 顯示指定測試入口，需輸入測試驗證碼、Google 登入及逐次雲端同意；目前已接上受限制的 staging endpoint，照片只會送到指定測試後端，完成實機驗收前不得改為 `public`。
