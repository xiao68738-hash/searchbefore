...
assert.match(manifest,/android:label="@string\/app_name"/,'launcher label must follow build variant');
assert.match(gradle,/defaultConfig\s*\{[\s\S]*?resValue "string", "app_name", "噴前查"/,'formal package uses the real product name');
assert.match(gradle,/debug\s*\{[^}]*resValue "string", "app_name", "噴前查・原生預覽"/,'side-by-side debug preview remains clearly identified');
