...
assert.doesNotMatch(html, /<script src="\.\/web-support-config\.js"><\/script>/, "綠界設定不可隨 Google Play App 靜態載入");
...
assert.match(html, /Google Play 版不提供外部付款/);
...
assert.match(html, /function openSponsorModal\(\)\{\s*if\(isInstalledAppContext\(\)\)\{clearSupportLinks\(\);return;\}/);
assert.match(html, /if\(isInstalledAppContext\(\)\)\{\s*clearSupportLinks\(\);\s*return;/);
