...
assert.doesNotMatch(html, /<script src="\.\/web-support-config\.js"><\/script>/, "綠界設定必須按需載入，保留 Google Play 遠端關閉能力");
...
assert.match(sw, /const CACHE_VERSION = "v0\.3\.9\.9-pest-taxonomy-search-sync-session-guard-plot-scope-2026-09-09"/);
...
assert.match(html, /有新版本可以更新/);
assert.match(html, /更新離線快取/);
assert.match(html, /title:"自願支持管道已開放"/);
...
assert.match(html, /id="homeGuide"/);
assert.match(html, /第一次使用，只要三步/);
assert.match(html, /雙向合法用藥查詢/);
assert.match(html, /queryMode==="agent"\?\(agentPicked\|\|agentQ\):\(selCrop\|\|cropQ\)/);
assert.match(html, /id="profileBetaCard"/);
assert.match(html, /請用一次真實流程，再告訴我們哪裡卡住/);
assert.doesNotMatch(html, /id="heroBox"|噴藥前查清楚/);
...
assert.match(html, /id="supportLink50"[^>]*rel="noopener noreferrer"/);
assert.match(html, /id="supportLink100"[^>]*rel="noopener noreferrer"/);
assert.match(html, /id="supportLink150"[^>]*rel="noopener noreferrer"/);
assert.match(html, /id="supportCard" style="display:none/);
assert.match(html, /所有使用者都享有相同功能與服務/);
assert.match(html, /不會解鎖內容、功能、雲端空間、徽章、優先服務或任何使用權益/);
assert.match(html, /除金流處理費外，款項均支付給噴前查開發者/);
assert.match(html, /function publicConfigSupportUrl\(key\)/);
assert.match(html, /function isGooglePlayAppContext\(\)/);
assert.match(html, /function rememberGooglePlayAppContext\(\)/);
assert.match(html, /function googlePlayVoluntarySupportEnabled\(\)/);
assert.match(html, /function supportAllowedForCurrentContext\(\)/);
assert.match(html, /function loadWebSupportConfig\(\)/);
assert.match(html, /script\.src="\.\/web-support-config\.js"/);
assert.match(html, /function clearSupportLinks\(\)/);
assert.match(html, /link\.removeAttribute\("href"\)/);
assert.match(html, /parsed\.hostname==="p\.ecpay\.com\.tw"/);
assert.match(html, /event\.stopImmediatePropagation\(\)/);
assert.match(html, /自願支持目前暫停；所有功能仍可免費使用/);
assert.match(html, /android-app:\/\/tw\.searchbefore\.app/);
assert.match(html, /params\.get\("app"\)==="google-play"/);
assert.match(html, /return supportAllowedForCurrentContext\(\)\?ANNOUNCE:ANNOUNCE\.filter/);
assert.match(html, /function refreshAnnouncementAvailability\(\)/);
assert.match(html, /refreshAnnouncementAvailability\(\);\s*resolve\(allowed\)/);
assert.match(html, /return googlePlayVoluntarySupportEnabled\(\);/);
assert.match(html, /onclick="openAnnouncementAction\('support'\)"/);
assert.match(html, /function openSponsorModal\(\)\{\s*loadWebSupportConfig\(\)\.then/);
assert.match(html, /if\(!supportAllowedForCurrentContext\(\)\)\{\s*clearSupportLinks\(\);\s*return;/);
assert.doesNotMatch(html, /function isInstalledAppContext\(\)/);
assert.match(html, /function publicConfigHttpsUrl\(key\)/);
assert.doesNotMatch(html, /支持一個月維護/);
...
assert.match(sw, /isRuntimeConfig.*service-config\|web-support-config/);
assert.doesNotMatch(sw, /PRECACHE\s*=\s*\[[\s\S]*"\.\/web-support-config\.js"/, "App 離線骨架不可預載綠界設定");
...
assert.match(sw, /v0\.3\.9\.9-pest-taxonomy-search-sync-session-guard-plot-scope/);
...
assert.match(about, /href="https:\/\/opencall\.searchbefore\.tw\/"/);
assert.match(about, /SearchBefore 旗下獨立維護的藝術機會整理專案/);
assert.match(about, /兩站的帳號、田間紀錄、收藏與搜尋資料不互通/);
