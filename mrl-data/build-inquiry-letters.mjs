...
/* ── 函二:動植物防疫檢疫署 ── */
/* 【2026-07-28 已結案，勿再據此發文】
   防檢署 防檢三字第1151842628號 函復，已澄清下列本函兩處錯誤：
   (1) pesticide.aphia.gov.tw 為防檢署「農藥資訊服務網」；
       「植物保護資訊系統」係農業藥物試驗所建置之另一個網站。
   (2) 該 6 種為早期公告藥劑，現行防檢署網站本來就未列英文名稱，
       並非開放資料匯出程序遺漏——本函第二段的前提不成立。
   該署已同意參酌國內外相關資訊補正英文名稱欄位。
   全文與後續動作見 mrl-data/詢問函-防檢署-回覆.txt */
...
二、經比對後確認，此為開放資料匯出程序遺漏欄位，而非登記資料
    本身缺漏：貴署「植物保護資訊系統」（pesticide.aphia.gov.tw）
    之許可證查詢頁面，該等藥劑之「英文普通名稱」欄位均有正確內容，
    惟同一資料於開放資料平臺之「英文名稱」欄位為空值。

    例證（查詢系統有值 → 開放資料為空）：
    ・許可證進字 01382「銅合硫磺」：查詢系統顯示
      copper oxychloride + sulfur，開放資料為空
    ・許可證進字 02172「波爾多」：查詢系統顯示
      bordeaux mixture，開放資料為空
    ・許可證製字 00785「克蠅香」：查詢系統顯示
      4-(p-acetoxyphenyl)-2-butanone + methyl eugenol，開放資料為空

三、此欄位缺漏造成之實務困難：

    衛生福利部「農藥殘留容許量標準」係以國際普通名稱（英文）為主要
    對照依據。開放資料英文名稱缺漏時，程式無法自動將該藥劑與殘留
    容許量標準勾稽。

四、下列 ${missingEn.length} 種目前仍有有效許可證，影響較為直接：

${missingEn.map(r => `    ${r.zh}（農藥代號 ${r.code}｜${r.kind}／${r.type}｜有效許可證 ${r.active} 張）`).join("\n")}

五、敬請檢查開放資料之匯出程序，將查詢系統既有之「英文普通名稱」
    一併輸出至開放資料之「英文名稱」欄位。
