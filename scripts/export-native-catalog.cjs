  ...
  for(const [crop,pests] of Object.entries(data)){
    ...
    for(const [pest,agents] of Object.entries(pests)){
      ...
      agents.forEach((a,index)=>{
        ...
        const cropForms={};
        for(const form of F.FORMS[crop]?.forms||[]){
          const split=F.splitIndices(crop,form.id,[a]),reminder=M.resolve(crop,a.name,{form:form.id});
          cropForms[form.id]={category:split.matched.length?'matched':split.excluded.length?'excluded':'unspecified',
            reason:split.excluded[0]?.reason||'',mrl:reminder?{status:reminder.status,scopeNote:reminder.scopeNote,evidence:reminder.evidence}:null};
        }
        rows.push({id:crypto.createHash('sha256').update(JSON.stringify([crop,pest,index,a])).digest('hex').slice(0,24),crop,pest,...a,
          ...
          usage,formKind:S.formKind(a.form),seed:A.isSeedTreatment(a),cropForms,
          mrl:mrl?{status:mrl.status,label:mrl.label||'',evidence:mrl.evidence||''}:null});
      });
    }
  }
  const terms=Array.from(new Set(rows.flatMap(r=>[r.crop,r.name,...(r.bl||[])]).concat(Object.keys(F.FORM_ALIAS))));
  const readings=Object.fromEntries(terms.map(t=>[t,A.readingProfile(t)]));
...
if(require.main===module){
  ...
  require('./export-native-goldens.cjs').writeGoldens();
}
